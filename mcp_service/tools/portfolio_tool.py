...
class PortfolioTool:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Initialize agents - check if already instantiated or need to create
        portfolio_agent_config = config.get("portfolio_agent", {})
        if isinstance(portfolio_agent_config, PortfolioAgent):
            self.portfolio_agent = portfolio_agent_config
        else:
            self.portfolio_agent = PortfolioAgent(portfolio_agent_config)
        
        risk_agent_config = config.get("risk_agent", {})
        if isinstance(risk_agent_config, RiskAgent):
            self.risk_agent = risk_agent_config
        else:
            self.risk_agent = RiskAgent(risk_agent_config)
