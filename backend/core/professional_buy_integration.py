...
class ProfessionalBuyIntegration:
    ...
    def __init__(self, config: Dict):
        ...
        self.enable_professional_logic = config.get(
            "enable_professional_buy_logic", True)
        # Changed to False for consistency
        self.fallback_to_legacy = config.get("fallback_to_legacy_buy", False)
    ...
    def evaluate_professional_buy(
        ...
        enable_buy = str(os.getenv("ENABLE_BUY", "true")
                         ).lower() not in ("false", "0", "no", "off")
        if not enable_buy:
            logger.info(
                f"❌ BUY DISABLED: Buy signals globally disabled by configuration (ENABLE_BUY=false) for {ticker}")
            return {
                "action": "hold",
                "ticker": ticker,
                "qty": 0,
        ...
        try:
            logger.info(
                f"=== STARTING PROFESSIONAL BUY EVALUATION FOR {ticker} ===")
            ...
            logger.info(
                f"  Signals Triggered: {len(buy_decision.signals_triggered)}")
            logger.info(f"  Reasoning: {buy_decision.reasoning}")

            # Convert to legacy format
            result = self._convert_to_legacy_format(
                buy_decision, stock_metrics, portfolio_context)
            ...
            if result['action'] == 'buy':
                logger.info(
                    f"🟢 BUY DECISION FINALIZED: {result['qty']} shares of {ticker}")
                ...
                logger.info(
                    f"   Professional Reasoning: {result['professional_reasoning']}")
                ...
                logger.info(
                    f"🔴 UNEXPECTED DECISION: {result['action']} for {ticker}")

            logger.info(
                f"=== END PROFESSIONAL BUY EVALUATION FOR {ticker} ===")

            return result

        except Exception as e:
            logger.error(
                f"Error in professional buy evaluation for {ticker}: {e}")
    ...
    def _build_stock_metrics(
        ...
        if price_history is not None and len(price_history) > 1:
            ...
            if 'ATR' in price_history.columns:
                atr_value = price_history['ATR'].iloc[-1] if not pd.isna(
                    price_history['ATR'].iloc[-1]) else 0.0
        ...
        sma_20 = safe_float(technical.get(
            "sma_20", current_price), current_price)
        sma_50 = safe_float(technical.get(
            "sma_50", current_price), current_price)
        sma_200 = safe_float(technical.get(
            "sma_200", current_price), current_price)
        support_level = safe_float(technical.get(
            "support_level", current_price * 0.95), current_price * 0.95)
        resistance_level = safe_float(technical.get(
            "resistance_level", current_price * 1.05), current_price * 1.05)
        volume_ratio = safe_float(technical.get("volume_ratio", 1.0), 1.0)

        # Extract fundamental metrics with defaults and type conversion
        fundamental_data = analysis_data.get(
            "fundamental_analysis", {}) or analysis_data.get("fundamental_metrics", {})
        price_to_book = safe_float(
            fundamental_data.get("price_to_book", 2.0), 2.0)
        price_to_earnings = safe_float(
            fundamental_data.get("price_to_earnings", 15.0), 15.0)
        earnings_growth = safe_float(fundamental_data.get(
            "earnings_growth", 0.05), 0.05)  # Default 5% growth
        return_on_equity = safe_float(fundamental_data.get(
            "return_on_equity", 0.10), 0.10)  # Default 10% ROE
        free_cash_flow_yield = safe_float(fundamental_data.get(
            "free_cash_flow_yield", 0.05), 0.05)  # Default 5% FCF yield
        debt_to_equity = safe_float(fundamental_data.get(
            "debt_to_equity", 0.5), 0.5)  # Default 0.5 debt-to-equity
        dividend_yield = safe_float(fundamental_data.get(
            "dividend_yield", 0.0), 0.0)  # Default 0% dividend yield
        payout_ratio = safe_float(fundamental_data.get(
            "payout_ratio", 0.0), 0.0)  # Default 0% payout ratio
        earnings_quality = safe_float(fundamental_data.get(
            "earnings_quality", 0.5), 0.5)  # Default 50% earnings quality
        insider_ownership = safe_float(fundamental_data.get(
            "insider_ownership", 0.0), 0.0)  # Default 0% insider ownership
        sector_pe = safe_float(fundamental_data.get(
            "sector_pe", 20.0), 20.0)  # Default sector P/E of 20
    ...
    def _convert_to_legacy_format(self, buy_decision: BuyDecision, stock_metrics: StockMetrics, portfolio_context: Dict = None) -> Dict:
        ...
        if portfolio_context and "available_cash" in portfolio_context and "total_value" in portfolio_context:
            ...
            # Get product type to determine leverage
            product_type = portfolio_context.get("product_type", "CNC")

            # Apply leverage for intraday (MIS) positions
            # Dhan provides 5x leverage for MIS intraday trades
            ...
            logger.info(
                f"Available Cash: Rs.{available_cash:.2f}, Effective Buying Power: Rs.{effective_buying_power:.2f}")

            # Calculate target position value based on position scale
            position_scale = buy_decision.buy_percentage
            target_position_value = total_value * position_scale * \
                0.1  # Limit to 10% of portfolio for safety

            # Ensure we don't exceed effective buying power (with leverage)
            target_position_value = min(
                target_position_value, effective_buying_power)
            ...
            if current_price > 0:
                ...
                # Verify the trade value is within limits
                trade_value = qty * current_price
                if trade_value > effective_buying_power:
                    logger.warning(
                        f"Trade value Rs.{trade_value:.2f} exceeds buying power Rs.{effective_buying_power:.2f}, reducing quantity")
                    qty = max(1, int(effective_buying_power / current_price))
    ...
    def _track_signals(self, buy_decision: BuyDecision, stock_metrics: StockMetrics):
        ...
        try:
            ...
            logger.debug(
                f"Tracked {len(buy_decision.signals_triggered)} signals for continuous learning")
